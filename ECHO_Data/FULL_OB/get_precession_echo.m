function echo=get_precession_echo(parameter,RCS,bias,snr)
...
sk=zeros(Coh_pulse_num*Tsim,64);
...
Psignal = sum(sum((sk.*conj(sk))))/(Coh_pulse_num*Tsim*64);
Pnoise = Psignal/(10^(snr/10));
sigma = sqrt(Pnoise/2);
echo = sk + sigma*randn(size(sk))+1j*sigma*randn(size(sk));
